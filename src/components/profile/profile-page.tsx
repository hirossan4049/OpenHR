...
import { Github, Mail, User, Trophy, Calendar, Briefcase, FileText, ExternalLink, PlusCircle, Pencil, Users } from "lucide-react";
...
export function ProfilePage() {
  ...
  const { data: myOrganizedProjects } = api.project.getMyProjects.useQuery(undefined, {
    enabled: status === "authenticated"
  });
  const { data: myParticipatingProjects } = api.project.getMyParticipatingProjects.useQuery(undefined, {
    enabled: status === "authenticated"
  });
              ...
              <div className="space-y-6">
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>あなたが主催しているプロジェクト</CardDescription>
                    <div className="flex gap-2 pt-2">
                  ...
                  <CardContent>
                    {!myOrganizedProjects || myOrganizedProjects.length === 0 ? (
                      <p className="text-muted-foreground">主催しているプロジェクトはありません。</p>
                    ) : (
                      <div className="space-y-4">
                        {myOrganizedProjects.map((project) => (
                          <div
                            key={project.id}
                              ...
                              <div className="flex-1">
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                  <span className="text-xs px-2 py-0.5 bg-muted rounded">
                                    {project.type === "hackathon" ? "ハッカソン" :
                                     project.type === "event" ? "イベント" : "プロジェクト"}
                                  </span>
                                  <span className={`text-xs px-2 py-0.5 rounded ${
                                    ...
                                    {project.recruitmentStatus === "open" ? "募集中" : "募集終了"}
                                  </span>
                                </div>
                                {project.description && (
                                ...
                                <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
                                  <div className="flex items-center gap-1">
                                    ...
                                    {project._count.members}人参加
                                  </div>
                                  {project._count.applications > 0 && (
                                    <span className="text-orange-600">
                                      {project._count.applications}件の未処理申請
                                    </span>
                                  )}
                                </div>
                              </div>
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>メンバーとして参加しているプロジェクト</CardDescription>
                  </CardHeader>
                  <CardContent>
                    {!myParticipatingProjects || myParticipatingProjects.length === 0 ? (
                      <p className="text-muted-foreground">参加中のプロジェクトはありません。</p>
                    ) : (
                      <div className="space-y-4">
                        {myParticipatingProjects.map((project) => (
                          <div
                            key={project.id}
                            className="border rounded-lg p-4 hover:bg-muted/50 transition-colors"
                          >
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                ...
                                <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
                                  {project.organizer && (
                                    <div className="flex items-center gap-1">
                                      主催: {project.organizer.name || "Unknown"}
                                    </div>
                                  )}
                                  {project.joinedAt && (
                                    <div className="flex items-center gap-1">
                                      <Calendar className="h-3 w-3" />
                                      {new Date(project.joinedAt).toLocaleDateString()}から参加
                                    </div>
                                  )}
                                </div>
                              </div>
